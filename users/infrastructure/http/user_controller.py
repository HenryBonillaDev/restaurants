...
@extend_schema(
    request=LoginSerializer, responses={200: "Login successful"}, methods=["POST"]
)
class Login(APIView):
    permission_classes = [AllowAny]
...
@extend_schema(request=ChangePasswordSerializer, methods=["POST"])
...
class UserController(APIView):
    ...
    def get(self, request):
        ...
        data = [
            {"id": user.id, "nombre": user.first_name, "role": user.role}
            for user in users
        ]
